package com.hireikon.hireikon_backend.service

import org.springframework.beans.factory.annotation.Value
import org.springframework.http.MediaType
import org.springframework.stereotype.Service
import org.springframework.web.reactive.function.client.WebClient
import org.springframework.web.reactive.function.client.bodyToMono
import java.time.Year

@Service
class EmailService(
    @Value("\${app.resend.api-key}") private val apiKey: String,
    @Value("\${app.resend.from}") private val fromAddress: String,
    @Value("\${app.base-url}") private val baseUrl: String
) {

    private val webClient = WebClient.builder()
        .baseUrl("https://api.resend.com")
        .defaultHeader("Authorization", "Bearer $apiKey")
        .defaultHeader("Content-Type", MediaType.APPLICATION_JSON_VALUE)
        .build()

    fun sendPasswordResetEmail(
        toEmail: String,
        rawToken: String,
        fullName: String,
    ) {
        val resetLink = "$baseUrl/reset-password?token=$rawToken"
        val html = buildResetEmailHtml(fullName, resetLink)
        sendEmail(
            to = toEmail,
            subject = "Reset your Hireikon Password",
            html = html
        )
    }

    private fun sendEmail(
        to: String,
        subject: String,
        html: Any
    ) {
        val body = mapOf(
            "from" to fromAddress,
            "to" to listOf(to),
            "subject" to subject,
            "html" to html
        )

        try {
            webClient.post()
                .uri("/emails")
                .bodyValue(body)
                .retrieve()
                .onStatus({ it.isError }) { res ->
                    res.bodyToMono<String>().map { responseBody ->
                        RuntimeException("Resend API error ${res.statusCode()}: $responseBody")
                    }
                }
                .bodyToMono<String>()
                .block()
        } catch (ex: Exception) {
            println("Warning: Failed to send email to $to: ${ex.message}")
            throw RuntimeException("Failed to send email. Please try again later.")
        }
    }

    private fun buildResetEmailHtml(fullName: String, resetLink: String): String = """
        <!DOCTYPE html>
        <html lang="en">
        <head>
            <meta charset="UTF-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
            <title>HireIkon - Reset your password</title>
        </head>
        <body style="margin:0;padding:0;background:#f4f4f5;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
        <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f4f5;padding:40px 0;">
            <tr>
                <td align="center">
                    <table width="520" cellpadding="0" cellspacing="0"
                           style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">

                        <!-- Header -->
                        <tr>
                            <td style="background:#1a1a2e;padding:32px 40px;text-align:center;">
                                <h1 style="margin:0;color:#ffffff;font-size:24px;font-weight:700;letter-spacing:-0.5px;">Hireikon</h1>
                            </td>
                        </tr>

                        <!-- Body -->
                        <tr>
                            <td style="padding:40px 40px 32px;">
                                <h2 style="margin:0 0 12px;color:#111827;font-size:20px;font-weight:600;">Reset your password</h2>
                                <p style="margin:0 0 24px;color:#6b7280;font-size:15px;line-height:1.6;">
                                    Hi $fullName,<br><br>
                                    We received a request to reset the password for your Hireikon account.
                                    Click the button below to choose a new password.
                                </p>

                                <!-- Button -->
                                <table cellpadding="0" cellspacing="0" style="margin:0 0 24px;">
                                    <tr>
                                        <td style="background:#1a1a2e;border-radius:8px;">
                                            <a href="$resetLink"
                                               style="display:inline-block;padding:14px 32px;color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;letter-spacing:0.2px;">
                                                Reset Password
                                            </a>
                                        </td>
                                    </tr>
                                </table>

                                <p style="margin:0 0 8px;color:#6b7280;font-size:13px;line-height:1.6;">
                                    This link expires in <strong>15 minutes</strong>.
                                </p>
                                <p style="margin:0 0 24px;color:#6b7280;font-size:13px;line-height:1.6;">
                                    If you didn't request a password reset, you can safely ignore this email.
                                    Your password won't be changed.
                                </p>

                                <!-- Fallback link -->
                                <div style="background:#f9fafb;border-radius:8px;padding:16px;border:1px solid #e5e7eb;">
                                    <p style="margin:0 0 6px;color:#6b7280;font-size:12px;">
                                        Button not working? Copy and paste this link:
                                    </p>
                                    <p style="margin:0;word-break:break-all;font-size:12px;">
                                        <a href="$resetLink" style="color:#1a1a2e;">$resetLink</a>
                                    </p>
                                </div>
                            </td>
                        </tr>

                        <!-- Footer -->
                        <tr>
                            <td style="padding:0 40px 32px;border-top:1px solid #f3f4f6;">
                                <p style="margin:24px 0 0;color:#9ca3af;font-size:12px;line-height:1.6;text-align:center;">
                                    © ${Year.now().value} Hireikon. All rights reserved.<br>
                                    This is an automated email — please do not reply.
                                </p>
                            </td>
                        </tr>

                    </table>
                </td>
            </tr>
        </table>
        </body>
        </html>
    """.trimIndent()
}